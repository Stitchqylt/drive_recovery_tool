  ...
  <div class="container">
    ...
    <div class="grid-2">
      <div class="card">
        ...
        <div style="font-size: 10pt; font-weight: 600; color: #0f172a; margin-bottom: 4px;">{drive_info.get("name", "Physical Drive")}</div>
        <div style="font-size: 9pt; color: #64748b;">Device Path: <code>{drive_info.get("device_path", "\\\\.\\PhysicalDriveX")}</code></div>
      ...
      <div class="card">
        ...
        <div style="font-size: 8.5pt; color: #475569;">Reallocated Sectors: <strong>{smart_report.reallocated_sectors if smart_report else 'N/A'}</strong></div>
        <div style="font-size: 8.5pt; color: #475569;">Pending Bad Sectors: <strong>{smart_report.pending_sectors if smart_report else 'N/A'}</strong></div>
        <div style="font-size: 8.5pt; color: #475569;">Drive Temperature: <strong>{smart_report.temperature_c if smart_report else 'N/A'} &deg;C</strong></div>
      </div>
    </div>
    ...
    <div class="grid-4">
      <div class="card" style="text-align: center;">
        ...
        <div class="card-value" style="color: #10b981;">{rec_pct:.1f}%</div>
        <div style="font-size: 8pt; color: #64748b;">{rec_files + part_files} / {tot_files} Files</div>
      </div>

      <div class="card" style="text-align: center;">
        <div class="card-title">100% Healthy</div>
        <div class="card-value" style="color: #059669;">{rec_files}</div>
        <div style="font-size: 8pt; color: #64748b;">Zero bad sectors</div>
      </div>

      <div class="card" style="text-align: center;">
        <div class="card-title">Partial (0-Filled)</div>
        <div class="card-value" style="color: #d97706;">{part_files}</div>
        <div style="font-size: 8pt; color: #64748b;">Preserved good data</div>
      </div>

      <div class="card" style="text-align: center;">
        <div class="card-title">Bad Sectors Hit</div>
        <div class="card-value" style="color: #dc2626;">{stats.get("bad_sectors", 0):,}</div>
...
"""
    ...
    for rec in manifest_records[:500]:  # Limit to 500 records for printable doc
        ...
        html += f"""        <tr>
          <td><strong>{rec.get("OriginalPath", "Unnamed")}</strong></td>
          <td><span class="badge {badge_cls}">{status}</span></td>
          <td>{size_str}</td>
          <td><span class="hash">{sha_hash[:16]}...{sha_hash[-12:] if len(sha_hash) > 28 else ''}</span></td>
        </tr>\n"""

    if len(manifest_records) > 500:
        html += f"""        <tr>
          <td colspan="4" style="text-align: center; color: #64748b; font-style: italic;">
            ... and {len(manifest_records) - 500:,} additional records logged in recovery_manifest.csv
          </td>
        </tr>\n"""
